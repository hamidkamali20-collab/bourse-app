...
ALL = ["فملی","فولاد","فخوز","ذوب","هرمز","کاوه","ارفع","کگل","کچاد","ومعادن","کنور","کروی","کمنگنز","فزر","کامافزر","طلا","عیار","کهربا","گوهر","زر","مثقال"]

def get_symbol(l18):
    try:
        r = requests.get(f"{BASE}/Tsetmc/Symbol.php?key={BRS_KEY}&l18={l18}", headers=HEADERS, timeout=15)
        j = r.json()
        # اگر brsapi ارور داد - مستقیم از خود tsetmc بگیر
        if isinstance(j, dict) and j.get("successful")==False:
            return {"error": j.get("error_message"), "pc":0}
        return j
    except Exception as e:
        return {"error": str(e), "pc":0}
...
def calc_signal(hist, data):
    try:
        if len(hist)>=10:
            closes=[]
            for x in hist[-30:]:
                v=x.get('pc') or x.get('close') or x.get('c') or x.get('p')
                if v: closes.append(float(v))
            s=pd.Series(closes)
            price=closes[-1]
        else:
            price=float(data.get('pc') or data.get('pl') or data.get('py') or 0)
            if price==0: return {"error":"قیمت نامشخص - سهمیه امروز تموم شده"}
        ...
        t1=price + atr*2
        t2=price + atr*3
        breakeven=price*1.011
        ma20=s.rolling(20).mean().iloc[-1] if len(s)>=20 else s.mean()
        sig="خرید ✅" if price>ma20*1.01 else "فروش ❌" if price<ma20*0.99 else "خنثی ➖"
        return {"price":round(price), "ma20":round(ma20), "stop":round(stop), "t1":round(t1), "t2":round(t2), "be":round(breakeven), "sig":sig}
    except Exception as e:
        return {"error":str(e)}
...
def get_codal(l18):
    # از خود BrsApi کدال
    try:
        # اندپوینت رسمی کدال BrsApi
        url=f"{BASE}/Codal/Codal.php?key={BRS_KEY}&l18={l18}"
        r=requests.get(url, headers=HEADERS, timeout=15).json()
        if isinstance(r, list): return r[:5]
        if isinstance(r, dict) and "data" in r: return r["data"][:5]
        return [str(r)[:400]]
    except Exception as e:
        return [f"کدال در پلن رایگان محدوده - برای فعالسازی کامل باید پلن کدال BrsApi بگیرید ({e})"]

def get_news():
    try:
        r=requests.get("https://www.sena.ir", headers=HEADERS, timeout=10)
        soup=BeautifulSoup(r.text,'html.parser')
        titles=[a.get_text().strip() for a in soup.select("a") if len(a.get_text().strip())>30][:6]
        return titles if titles else ["سنا فیلتر است"]
    except: return ["خطا در دریافت اخبار"]

def get_telegram_proxy(channel):
    # روش ضد فیلتر: اول مستقیم، بعد با پراکسی allorigins
    urls=[f"https://t.me/s/{channel}", f"https://api.allorigins.win/raw?url=https://t.me/s/{channel}", f"https://corsproxy.io/?https://t.me/s/{channel}"]
    for url in urls:
        try:
            r=requests.get(url, headers=HEADERS, timeout=15)
            if "tgme_widget_message_text" not in r.text: continue
            soup=BeautifulSoup(r.text,'html.parser')
            msgs=soup.select(".tgme_widget_message_text")
            if msgs: return [m.get_text()[:400] for m in msgs[:8]]
        except: continue
    return ["تلگرام با اینترنت ایران فیلتر است - برای حل دائمی اپ رو روی سرور خارج بذار (راهنما پایین)"]
...
def api_fund(name:str):
    d=get_symbol(name); return {"fund":get_fund(d), "codal":get_codal(name), "price":d.get('pc')}
@app.get("/api/telegram/{channel}")
def api_tg(channel:str): return {"channel":channel, "messages":get_telegram_proxy(channel)}
@app.get("/api/news")
...
<div class="header"><h2 style="margin:0">🚀 بورس پرو</h2><p style="opacity:.85;margin:4px 0;font-size:13px">فولاد | مس | طلا | معدنی - تکنیکال + فاندامنتال + کدال + اخبار + تلگرام ضدفیلتر</p></div>
...
<input id="sym" value="فملی" style="width:120px"><select id="chan"><option value="tahlilboniadii">tahlilboniadii</option><option value="bourse_signal_ir">bourse_signal</option><option value="boursenews_ir">boursenews</option><option value="pul_saz">pul_saz</option></select>
...
async function load(sym){
 ...
 try{
  ...
  if(d.error){document.getElementById('main').innerHTML='<div class=card style=color:red>'+d.error+'</div>';return;}
  ...
  <div class=grid style=margin-top:12px>
    ...
    <div><div class=label>حجم / ارزش (B)</div><div class=value style=font-size:13px>${(d.tvol/1000000).toFixed(1)}M<br>${(d.tval/1e12).toFixed(2)} همت</div></div>
  ...
  </div><div class=label>قیمت فعلی محاسبه: ${fmt(s.price||0)} ریال</div></div>`;
  ...
  document.getElementById('news').innerHTML='<b>🗞 اخبار اقتصادی/سیاسی</b><br>'+n.news.map(x=>'<div style=padding:6px 0;border-bottom:1px solid #f1f5f9;font-size:12px>'+x+'</div>').join('');
  let ch=document.getElementById('chan').value;
  let t=await fetch('/api/telegram/'+ch).then(r=>r.json());
  document.getElementById('tg').innerHTML='<b>✈️ تلگرام @'+t.channel+' (ضدفیلتر)</b><br>'+t.messages.map(m=>'<div style=padding:8px 0;border-bottom:1px solid #eee;font-size:13px>'+m+'</div>').join('');
