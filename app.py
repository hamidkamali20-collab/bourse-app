...
INSCODE = {"فملی":"35425587644337450","فولاد":"46348559193224090","فخوز":"27906419856424640","ذوب":"18488008641058109","هرمز":"15753803582221477","کگل":"35366681030756042","کچاد":"35700344742885862","ومعادن":"47466638396174110","فارس":"35425587644337450","شستا":"49568544329800661","اهرم":"53236819796184254","عیار":"35589735413538838","فزر":"17955987541249042","کروی":"48675780594344179"}
...
def get_fipiran(l18):
    try:
        urls=[f"https://www.fipiran.ir/Symbol?symbolpara={l18}", f"https://www.fipiran.com/Symbol?symbolpara={l18}"]
        for url in urls:
            ...
            eps=re.search(r"EPS\s*[:\s]*([-\d\.]+)", txt)
            return {"P/E فیپیران": pe.group(1) if pe else "ناموجود", "EPS فیپیران": eps.group(1) if eps else "ناموجود", "متن": txt[:800].strip().replace("\n"," ")[:400], "url":url}
...
def get_sahamyab(l18):
    try:
        ...
        soup=BeautifulSoup(r.text,'html.parser')
        tweets=[]
        for el in soup.select(".twContent, .content, .hashtag-tweet, .tweet-text, a[href*='/twit/']"):
            t=el.get_text().strip()
            if len(t)>20: tweets.append(t[:300])
        if not tweets:
            for a in soup.select("a"):
                t=a.get_text().strip()
                if len(t)>40: tweets.append(t[:300])
                if len(tweets)>=8: break
        uniq=[]
        for t in tweets:
            if t not in uniq: uniq.append(t)
        return uniq[:8] if uniq else ["توئیتی برای این نماد پیدا نشد"]
    except Exception as e: return [f"خطا سهام‌یاب: {e}"]
...
def screener():
    ...
    for s in TIER1+TIER2[:6]:
        ...
        score = int(f.get("score",0)) if str(f.get("score")).isdigit() else 0
...
<div class="header"><h2 style="margin:0">🚀 بورس پرو - ردیف 1 و 2 + فیپیران + سهام‌یاب</h2><p style="opacity:.85;font-size:12px">تابلوخوانی + فاندا BrsApi/فیپیران + حد ضرر هوشمند + غربالگر + سهام‌یاب پرامتیاز + نقد AI</p></div>
<div class="card"><b>ردیف 1:</b> <span class=tier>بزرگ</span><br>
...
<br><b style="margin-top:8px;display:inline-block">ردیف 2:</b> <span class=tier>پرنوسان</span><br>
<button class="btn" onclick="load('کگل')">کگل</button><button class="btn" onclick="load('کچاد')">کچاد</button><button class="btn" onclick="load('فزر')">فزر</button><button class="btn" onclick="load('ذوب')">ذوب</button><button class="btn" onclick="load('طلا')">طلا</button><button class="btn" onclick="load('عیار')">عیار</button><button class="btn" onclick="load('اهرم')">اهرم</button>
<hr><div style="display:flex;gap:6px;flex-wrap:wrap">
<input id="sym" value="فملی" style="width:100px"><select id="risk"><option value="low">کم ریسک</option><option value="normal" selected>متعادل</option><option value="high">پر ریسک</option></select><input id="custom" placeholder="حد ضرر دستی" style="width:130px"><select id="chan"><option value="tahlilboniadii">tahlilboniadii</option><option value="boursenews_ir">boursenews_ir</option></select><button onclick="load(document.getElementById('sym').value)" style="background:#1e40af;color:white;padding:8px 14px;border-radius:10px;border:none">نمایش + نقد</button><button onclick="loadScreener()" style="background:#059669;color:white;padding:8px 14px;border-radius:10px;border:none">غربالگر</button></div></div>
<div id="main"></div><div id="critics"></div><div id="tablo" class="card" style="display:none"></div><div id="screener" class="card" style="display:none"></div><div class="grid"><div id="fund" class="card"></div><div id="fipiran" class="card"></div></div><div class="grid"><div id="news" class="card"></div><div id="sahamyab" class="card"></div></div><div id="tg" class="card"></div>
<script>
...
async function load(sym){
 ...
 try{
  ...
  let fi=await fetch('/api/fipiran/'+sym).then(r=>r.json());
  ...
  document.getElementById('tablo').innerHTML=`<b>📊 تابلوخوانی</b><table><tr><td>قدرت</td><td><b>${t.قدرت || '-'}</b></td><td>سرانه خرید</td><td>${t.سرانه_خرید || '-'} M</td></tr><tr><td>پول حقیقی</td><td>${t.پول_حقیقی || '-'}</td><td>حجم</td><td>${t.حجم || '-'}</td></tr></table>`;
  document.getElementById('fund').innerHTML=`<b>📈 فاندا BrsApi</b><table><tr><td>P/E</td><td>${f.fund.pe}</td><td>P/E گروه</td><td>${f.fund.gpe}</td></tr><tr><td colspan=4><b>${f.fund.status}</b></td></tr></table><hr>${f.codal.map(x=>'<div style=font-size:11px>'+x+'</div>').join('')}`;
  document.getElementById('fipiran').innerHTML=`<b>🏛 فیپیران - fipiran.ir</b><br><div style=font-size:12px>P/E فیپیران: <b>${fi['P/E فیپیران']||fi.error||'-'}</b> | EPS: <b>${fi['EPS فیپیران']||'-'}</b></div><div style=font-size:11px;color:#64748b;margin-top:6px>${fi['متن']||fi.error||''}</div><a href="${fi.url||'#'}" target="_blank" style=font-size:11px>مشاهده در فیپیران</a>`;
  ...
  let sh=await fetch('/api/sahamyab/'+sym).then(r=>r.json()); document.getElementById('sahamyab').innerHTML='<b>💬 سهام‌یاب - پرامتیاز برای '+sh.نماد+'</b><br>'+sh.tweets.map(x=>'<div style=font-size:12px;padding:6px 0;border-bottom:1px solid #eee>'+x+'</div>').join('') + '<div style=font-size:10px;color:#64748b;margin-top:6px'><a href="https://r.sahamyab.com/hashtag/'+sym+'" target="_blank">مشاهده همه در سهام‌یاب</a></div>';
...
async function loadScreener(){
 ...
 let h='<b>🏆 غربالگر ردیف 1 و 2</b><table><tr><th>نماد</th><th>قیمت</th><th>وضعیت</th><th>قدرت</th><th>پول</th></tr>';
